            ...
            if bcd is not None:
                ...
                if self.face_vector is None and self.target_face_img is not None:
                    ...
                    for threshold in (0.5, 0.35, 0.2):
                        rects = self.face_detector.extract(self.target_face_img, threshold=threshold)[0]
                        if len(rects) != 0:
                            break
                    ...
                    if len(rects) > 0:
                        ...
                        lmrks_list = self.face_marker.extract(face_image)
                        if len(lmrks_list) > 0:
                            lmrks = lmrks_list[0]
                    ...
                    for i, fsi in enumerate(bcd.get_face_swap_info_list()):
                        if state.animator_face_id == i:
                            ...
                            if face_align_image is not None:
                                ...
                                anim_image = ImageProcessor(anim_image).resize((W,H), interpolation=ImageProcessor.Interpolation.LANCZOS4).get_image('HWC')
                                ...
                                # Use landmark hull as blend mask to avoid square-face blending artifacts.
                                if fsi.face_align_ulmrks is not None:
                                    blend_mask = fsi.face_align_ulmrks.get_convexhull_mask(anim_image.shape[:2], color=(255,), dtype=np.uint8)
                                else:
                                    blend_mask = np.full(anim_image.shape[:2], 255, dtype=np.uint8)

                                bcd.set_image(fsi.face_align_mask_name, blend_mask)
                                bcd.set_image(fsi.face_swap_mask_name, blend_mask)
